...
//
//		Log.d("MainAct", " screenWidth " + screenWidth + "  screenHeight" +  screenHeight );
	...
	private void initCamera2Support() {
        ...
        if( Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ) {
        	...
        	if( manager2.getNumberOfCameras() == 0 ) {
            	...
            	supports_camera2 = false;
				Log.d("RAN_EDIT","Camera2 reports 0 cameras, "+supports_camera2);
        	}
			Log.d("RAN_EDIT","return 1 " );
...
//        			if( MyDebug.LOG )
                	...
                	supports_camera2 = false;
					Log.d("RAN_EDIT","camera " + i + " doesn't have limited or full support for Camera2 API");
        		}
        		Log.d(TAG, " supports_camera2 i " + i);
        	}
        }
		Log.d("RAN_EDIT",""+supports_camera2);
		if( MyDebug.LOG )
			Log.d(TAG, "supports_camera2? " + supports_camera2);
	}
...
//		if( MyDebug.LOG )
    ...
    protected void onResume() {
		...
		{
            int resource;
			Log.d(TAG, " preview.isvideo " + preview.isVideo());
//			getMainUI().setTakePhotoIcon();

//            if (getPreview().isVideoRecording())
...
//		}
//		Immersive.CreateInstance().UiChangeListener(getWindow());
	}
    ...
    public void clickFlashAuto(View view){
		Log.d(TAG,"clickflash");
		mainUI.flashAuto();
	}

	public void clickHdrState(View view){
		Log.d(TAG,"clickflash");
		mainUI.hdrState();
	}

	public void clickFlashOff(View view){
		Log.d(TAG,"clickflashoff");
        mainUI.flashOff();
    }

	public void clickFlashOn(View view){
		Log.d(TAG,"clickflash");
		mainUI.flashOn();
	}
	...
	public void clickTimerOff(View view){
		Log.d(TAG,"clickTimerOff");
		mainUI.timer_off();
	}

	public void clickSecThree(View view){
		Log.d(TAG,"clickSecThree");
		mainUI.sec_three();
	}

	public void clickSecFive(View view){
		Log.d(TAG,"clickSecFive");
		mainUI.sec_five();
	}

	public void clickSecNine(View view){
		Log.d(TAG,"clickSecFive");
		mainUI.sec_nine();
	}

    public void clickBackSetting(View view){
		Log.d(TAG,"clickBackSetting");
		mainUI.back_setting();
	}

	public void clickSettingGear(View view){
		Log.d(TAG,"clickSettingGear");
		mainUI.setting_gear();
	}
...
//	public void clickedTakeVideo(View view) {
//		if( MyDebug.LOG )
//			Log.d(TAG, "clickedTakeVideo");
//		this.takeAV();
//	}
...
//		mainUI.setTakePhotoIcon();
    ...
    public void clickedSettingsDots(View view) {
//		if( MyDebug.LOG )
//			Log.d(TAG, "clickedSettings");
//		openSettings();

		mainUI.settings_dots();
    }
    ...
    public void openSettings() {
		...
		Log.d("MainAct", " MyPreferenceFragment ");
...
//						| View.SYSTEM_UI_FLAG_FULLSCREEN);
	...
	public void takeAV() {
		if( MyDebug.LOG )
			Log.d(TAG, "takeAV");
		closePopup();
		this.preview.startVideoRecording(true);
	}
...
//			takeVideoButton = findViewById(R.id.take_video);
			if( sharedPreferences.getBoolean(PreferenceKeys.getShowTakePhotoPreferenceKey(), true) ) {
				if( !mainUI.inImmersiveMode() ) {
					Log.d(TAG, " takePhotoButton vis");
					takePhotoButton.setVisibility(View.VISIBLE);
//					takeVideoButton.setVisibility(View.INVISIBLE);
				}
			}
			else {
				Log.d(TAG, " takePhotoButton inbi");
